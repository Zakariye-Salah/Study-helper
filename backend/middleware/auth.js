...
async function verifyToken(token) {
  ...
  // jwt.verify can throw; let caller handle
...
async function getUserFromPayload(payload) {
  ...
  if (!userId) {
    // nothing to look up in DB — return the token-derived fallback
    return tokenUser;
  }

  // Try to find a User (admins/managers)
  try {
    const dbUser = await User.findById(userId).select('-passwordHash').lean().exec().catch(() => null);
    if (dbUser) {
    ...
    // if DB lookup fails (connection), just continue to other collections
    // don't leak DB error here; callers will handle
  }
  ...
  if (normalizedRole === 'teacher') {
    ...
    // attach some teacher-specific fields so course logic can show teacher photo/title/bio easily
    return {
      _id: String(t._id),
      role: 'teacher',
  ...
  // Unknown role or token doesn't match any collection — return token fallback
  return {
    ...tokenUser,
 ...
 *
 * Note: This middleware returns 401/403 JSON responses similarly to your previous file.
 */
async function authMiddleware(req, res, next) {
  try {
    // extract token from several locations
    let token = null;
    const authHeader = req.headers['authorization'] || req.headers['Authorization'] || '';
    if (authHeader && typeof authHeader === 'string') {
      const parts = authHeader.trim().split(/\s+/);
    ...
    // cookie parser: req.cookies may exist if you use cookie-parser
    ...
    req.user = userObj;
    // keep token if routes/sockets want to use it
    req.token = token;
    return next();
  } catch (err) {
    console.error('Auth middleware error:', err && (err.stack || err));
...
/**
 * requireAuth middleware wrapper (for route usage)
 * Example: router.get('/counts', requireAuth, handler)
 */
async function requireAuth(req, res, next) {
  // call the main middleware and let it respond if unauthenticated
  return authMiddleware(req, res, next);
}

/**
 * requireRole(role) -> middleware to assert user role.
...
// Export: default middleware for use with app.use(authMiddleware),
// plus handy helpers for route-level imports.
