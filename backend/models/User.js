...
const UserSchema = new mongoose.Schema({
  fullname: { type: String, required: true, trim: true }, // school/university name (legacy)
  // NEW: manager personal name (keeps old 'fullname' for institution)
  fName: { type: String, trim: true, default: '' },

  email: { type: String, unique: true, sparse: true, trim: true, lowercase: true },
  phone: { type: String, trim: true, default: '' },

  // NEW: managerPhone as normalized digits-only string
  managerPhone: { type: String, trim: true, default: '' },

  // NEW: institution type: 'school' | 'university'
  institutionType: { type: String, enum: ['school','university'], default: null },

  // NEW: logo URL / path for institution logo
