 ...
 *
 * NOTE: replaced roles(...) gating with internal check for robust recognition of admin-like users.
 */
router.post('/competitions', auth, async (req, res) => {
  await safeHandler(async (req2, res2) => {
    if (!isAdminUser(req2.user)) return res2.status(403).json({ ok: false, message: 'Forbidden' });
 ...
 */
router.post('/competitions/:id/addPoints', auth, async (req, res) => {
  await safeHandler(async (req2, res2) => {
    if (!isAdminUser(req2.user)) return res2.status(403).json({ ok: false, message: 'Forbidden' });
    if (!Competition || !CompetitionResult) return res2.status(500).json({ ok: false, message: 'Competition models not available' });
 ...
 * body: { userId }
 */
router.post('/competitions/:id/clearPoints', auth, async (req, res) => {
  await safeHandler(async (req2, res2) => {
    if (!isAdminUser(req2.user)) return res2.status(403).json({ ok: false, message: 'Forbidden' });
    if (!Competition || !CompetitionResult) return res2.status(500).json({ ok: false, message: 'Competition models not available' });
    const compId = req2.params.id;
 ...
 * View competition result entries for user — admin/manager/teacher can view any, users can view their own.
 */
router.get('/competitions/:id/user/:userId/results', auth, async (req, res) => {
  await safeHandler(async (req2, res2) => {
    ...
    // Allow admin, manager, teacher OR the owner to view
    const requestingUser = req2.user || {};
    const ownerMatch = String(requestingUser._id) === String(userId);
    if (!(isAdminUser(requestingUser) || isManagerUser(requestingUser) || isTeacherUser(requestingUser) || ownerMatch)) {
      return res2.status(403).json({ ok: false, message: 'Forbidden' });
    }
 ...
 */
router.post('/competitions/:id/clearResult/:resultId', auth, async (req, res) => {
  await safeHandler(async (req2, res2) => {
    if (!isAdminUser(req2.user)) return res2.status(403).json({ ok: false, message: 'Forbidden' });
 ...
 */
router.post('/competitions/:id/clearAttempt/:attemptId', auth, async (req, res) => {
  await safeHandler(async (req2, res2) => {
    if (!isAdminUser(req2.user)) return res2.status(403).json({ ok: false, message: 'Forbidden' });
