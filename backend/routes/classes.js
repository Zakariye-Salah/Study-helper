...
async function generateClassId(schoolId) {
  ...
  const last = await Class.findOne({ schoolId, classId: new RegExp(`^${prefix}`) }).sort({ createdAt: -1 }).lean();
...
router.post('/', auth, roles(['admin','manager']), async (req,res)=>{
  try{
    ...
    // ensure unique classId for this school
    const existing = await Class.findOne({ classId, schoolId: req.user.schoolId });
...
router.get('/', auth, roles(['admin','manager','teacher','student']), async (req,res)=>{
  try{
    ...
    const items = await Class.find(q).limit(parseInt(limit)).skip((page-1)*limit).sort({ createdAt:-1 }).populate('subjectIds','name subjectId').lean();
...
router.get('/:id', auth, roles(['admin','manager','teacher','student']), async (req,res)=>{
  try{
    ...
    // find students in this class: if requester is student/teacher, restrict by school; for owners restrict by createdBy
    const sQuery = { classId: cls._id };
    if (['student','teacher'].includes(req.user.role)) {
      sQuery.schoolId = req.user.schoolId;
    } else {
      sQuery.createdBy = req.user._id;
    }
    const students = await Student.find(sQuery).select('fullname numberId phone status photo photoUrl').lean();
...
router.put('/:id', auth, roles(['admin','manager']), async (req,res)=>{ 
  try{
    ...
    if (classId && String(classId).trim() !== doc.classId) {
      const exists = await Class.findOne({ classId: String(classId).trim(), schoolId: req.user.schoolId });
    ...
    if (req.body.subjectIds && !Array.isArray(req.body.subjectIds)) {
      req.body.subjectIds = Array.isArray(req.body.subjectIds) ? req.body.subjectIds : (req.body.subjectIds ? [req.body.subjectIds] : []);
    }

    const c = await Class.findByIdAndUpdate(id, req.body, { new:true });
    res.json(c);
  }catch(err){
...
router.post('/:id/move', auth, async (req, res) => {
  try {
    ...
    if (!src || !dest) return res.status(404).json({ message: 'Source or target class not found' });
    ...
    let studentsToMoveQuery = { classId: sourceClassId };
    if (!moveAll) {
    ...
    try {
      ...
      // Optionally create Promotion/Audit entries here within the same session
