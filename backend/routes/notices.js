 ...
 */
/**
 * resolveRecipients({ targetRoles, explicitUserIds, schoolId, requesterRole, requesterId })
 * returns array of { userId, role }
 * requesterRole/requesterId optional - used to restrict scope for managers
 */
async function resolveRecipients({ targetRoles = [], explicitUserIds = [], schoolId, requesterRole = '', requesterId = null }) {
  const recipients = [];
  const seen = new Set();
  ...
  // Defensive: if requester is manager, remove 'manager' from targetRoles (managers cannot target managers)
  if ((requesterRole || '').toLowerCase() === 'manager') {
  ...
  // role-based resolution
  if (targetRoles.includes('student')) {
    // If requester is manager, restrict to students createdBy that manager (assumes Student has createdBy)
    const q = {};
    if (schoolId) q.schoolId = schoolId;
    if ((requesterRole || '').toLowerCase() === 'manager' && requesterId) q.createdBy = toObjectIdIfPossible(requesterId);
    const students = await Student.find(q).select('_id').lean().exec().catch(() => []);
    students.forEach(s => { const sid = String(s._id); if (!seen.has(sid)) { seen.add(sid); recipients.push({ userId: s._id, role: 'student' }); }});
  }
  if (targetRoles.includes('teacher')) {
    // If requester is manager, restrict to teachers createdBy that manager (assumes Teacher has createdBy)
    const q = {};
    if (schoolId) q.schoolId = schoolId;
    if ((requesterRole || '').toLowerCase() === 'manager' && requesterId) q.createdBy = toObjectIdIfPossible(requesterId);
    const teachers = await Teacher.find(q).select('_id').lean().exec().catch(() => []);
  ...
  if (otherRoles.length) {
    const roleRegex = otherRoles.map(r => new RegExp('^' + escapeRegex(r) + '$', 'i'));
    let users = [];
    // Admin can scope by school if provided; manager might be allowed to include admin role, we already removed 'manager' earlier
    if (schoolId) {
      try {
        users = await User.find({
          schoolId,
  ...
  console.info('resolveRecipients -> explicit:', explicitUserIds, 'targetRoles:', targetRoles, 'schoolId:', schoolId, 'requesterRole:', requesterRole, 'resolvedCount:', recipients.length);
  return recipients;
}
...
router.post('/debug/resolve', auth, async (req, res) => {
  try {
    ...
    const recs = await resolveRecipients({
      targetRoles,
      explicitUserIds,
      schoolId: req.user && req.user.schoolId ? req.user.schoolId : null,
      requesterRole: (req.user && req.user.role || '').toLowerCase(),
