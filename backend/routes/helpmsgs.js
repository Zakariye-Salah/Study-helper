...
router.post('/', auth, async (req, res) => {
  try {
    ...
    if (!thread) {
      thread = new HelpThread({ userId, courseId: body.courseId ? toObjectIdIfPossible(body.courseId) : null, subject: body.subject || '' });
      await thread.save();
    }
    ...
    // notify admin
    try { const io = req.app.get('io'); if (io) io.to('admins').emit('help:new', { threadId: thread._id, userId }); } catch(e){}
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    if (role === 'admin') {
      // group threads by user with unread counts
      const threads = await HelpThread.find({}).sort({ lastUpdatedAt: -1 }).lean();
      // for each thread, compute unread messages
      const out = [];
...
router.get('/:threadId/messages', auth, async (req, res) => {
  try {
    ...
    // mark messages read if admin viewing or user viewing messages that were to them
    if (role === 'admin') {
