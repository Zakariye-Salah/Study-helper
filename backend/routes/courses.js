...
const Lesson = require('../models/Lesson');
let Purchase = null;
try { Purchase = require('../models/Purchase'); } catch (e) { Purchase = null; }
...
router.get('/categories', async (req, res) => {
  try {
    ...
    const categories = (cats || []).map(c => c._id).filter(Boolean);
    if (!categories.length) return res.json({ ok:true, categories: ['Mobile Repairing', 'Languages', 'Subjects', 'Programming', 'Business', 'Design'] });
    return res.json({ ok:true, categories });
  } catch (err) {
    console.error('GET /courses/categories error', err && (err.stack||err));
    return res.status(500).json({ ok:false, error: 'Server error' });
...
router.get('/', async (req, res) => {
  try {
    ...
    // compute buyersCount using Purchase if present
    const courseIds = (docs || []).map(d=>d._id).filter(Boolean);
    let counts = [];
    if (Purchase && courseIds.length) {
      const validIds = courseIds.filter(id => mongoose.isValidObjectId(id)).map(id => mongoose.Types.ObjectId(id));
      if (validIds.length) {
        counts = await Purchase.aggregate([
          { $match: { courseId: { $in: validIds }, status: 'verified' } },
    ...
    const courses = (docs || []).map(c => {
      ...
      return {
        ...
        teacherId: c.teacherId || null,
        createdAt: c.createdAt || c.createdAt,
        enrolled: Array.isArray(c.enrolled) ? c.enrolled : []
      };
    });
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const buyersCount = (Purchase) ? await Purchase.countDocuments({ courseId: course._id, status: 'verified' }).catch(()=>0) : (course.buyersCount || 0);
    course.buyersCount = Number(buyersCount || course.buyersCount || 0);
    course.discountedPrice = computeDiscounted(Number(course.price || 0), Number(course.discount || 0));
...
router.post('/', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    // emit creation
    try { const io = req.app && req.app.get && req.app.get('io'); if (io) io.emit('course:created', { courseId: doc._id, course: doc }); } catch (e) { console.warn('emit failed', e); }
...
router.put('/:id', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    if ('teacherId' in body) course.teacherId = body.teacherId;
    if ('isFree' in body) course.isFree = !!body.isFree;
    if ('price' in body) course.price = Number(body.price || 0);
    if ('discount' in body) course.discount = Number(body.discount || 0);
    if ('duration' in body) course.duration = body.duration || course.duration || '';
    if ('durationMonths' in body) course.durationMonths = Number(body.durationMonths || course.durationMonths || 0);
    course.shortDescription = body.shortDescription || course.shortDescription || '';
    course.longDescription = body.longDescription || course.longDescription || '';
    course.thumbnailUrl = body.thumbnailUrl || course.thumbnailUrl || '';
    course.media = Array.isArray(body.media) ? body.media : course.media;
    ...
    try { const io = req.app && req.app.get && req.app.get('io'); if (io) io.emit('course:updated', { courseId: course._id, course }); } catch(e){}
...
router.delete('/:id', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    course.deletedBy = req.user._id;
    await course.save();
    try { const io = req.app && req.app.get && req.app.get('io'); if (io) io.emit('course:deleted', { courseId: course._id }); } catch(e){}
    return res.json({ ok:true, message: 'Soft-deleted' });
  } catch (err) {
    console.error('DELETE /courses/:id error', err && (err.stack || err));
    return res.status(500).json({ ok:false, error: 'Server error' });
