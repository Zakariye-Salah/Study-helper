...
/**
 * GET /api/courses
 * (keeps your existing behavior: returns paged list)
 */
router.get('/', auth, async (req, res) => {
  try {
    const q = (req.query.q || '').trim();
    ...
    const match = { deleted: { $ne: true }, visibility: { $ne: 'private' } };
...
/**
 * GET /api/courses/:id
 * returns course by _id or courseId, as before
 */
router.get('/:id', auth, async (req, res) => {
  try {
    const id = req.params.id;
 ...
 * (kept basic + thumbnail/teacher fields)
 */
router.post('/', auth, roles(['admin']), async (req, res) => {
  try {
    const body = req.body || {};
...
/**
 * DELETE /api/courses/:id/permanent (admin)
 * Removes course + optionally related objects
 */
router.delete('/:id/permanent', auth, roles(['admin']), async (req, res) => {
  try {
    const id = req.params.id;
    ...
    const purchaseCount = await PurchaseAttempt.countDocuments({ courseId: c._id, status: 'verified' }).catch(()=>0);
    if (purchaseCount > 0 && !(req.query.force === '1' || req.query.force === 'true')) {
      return res.status(400).json({ ok: false, message: 'Course has verified purchases. Pass ?force=1 to permanently delete.' });
    }
